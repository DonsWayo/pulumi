...
All deployment configuration is managed through Pulumi programs in `packages/infra/index.ts`. This provides type-safe infrastructure configuration with full programmatic control.
...
// Create AWS provider with specific region
...
const site = new Next("my-app", {
  ...
  // Lambda configuration
  lambda: {
    memory: config.getNumber("lambdaMemory") || 1024,
    timeout: config.getNumber("lambdaTimeout") || 30,
    architecture: "arm64",
  },

  // Domain configuration
  domain: config.getObject("domain") ? {
    name: config.require("domain.name"),
    certificateArn: config.get("domain.certificateArn"),
  } : undefined,
}, { provider });
...
config:
  ...
  my-app:nodeEnv: development
  my-app:apiUrl: http://localhost:3001
  my-app:lambdaMemory: 512

# Pulumi.staging.yaml
config:
  aws:region: us-east-1
  my-app:nodeEnv: staging
  my-app:apiUrl: https://staging-api.example.com
  my-app:lambdaMemory: 1024

# Pulumi.production.yaml
config:
  aws:region: us-east-1
  my-app:nodeEnv: production
  my-app:apiUrl: https://api.example.com
  my-app:lambdaMemory: 2048
  my-app:domain:
    name: app.example.com
    certificateArn: arn:aws:acm:us-east-1:...
```
